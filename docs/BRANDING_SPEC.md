...
*(เพิ่ม 2026-09-11 — มาสคอตเป็นตัวละครของ "ตัวเว็บ" ไม่ใช่ส่วนหนึ่งของ CI องค์กร)*

- render ผ่าน `components/layout/brand-mascot.tsx` จุดเดียว วาดด้วย semantic token ล้วน ไม่มี hex
- **ห้ามลอกมาสคอตจาก `water.jpg` ของ template** (ข้อ 2) — ตัวปัจจุบันออกแบบใหม่ทั้งหมด
- **ใช้ได้เฉพาะพื้นที่ chrome** — หน้า `/login`, หน้า 404, empty state
  **ห้ามวางในพื้นที่ข้อมูล** เพราะหมวกเป็นสีแดง ซึ่งในพื้นที่ข้อมูลแปลว่า "วิกฤต" เท่านั้น (ข้อ 3.3)
- ต้องอยู่นอก clear space ของโลโก้องค์กรเสมอ ห้ามวางติดกันจนดูเป็นชุดเดียว (ข้อ 5.2)
- มีสองชุดสี: `on-brand` (ตัวขาว สำหรับพื้นสีเข้ม) และ `on-surface` (ตัวสีน้ำ สำหรับพื้นการ์ด)
  เพราะตัวขาวจะหายไปเลยบนพื้นการ์ดสีขาวในโหมดสว่าง
