...
>
> **บันทึกการตัดสินใจ 2026-09-11** — ข้อ 3.3, 3.5, 3.6, 3.7, 4, 5.1, 5.3 และ 8 ถูกเติมด้วยคำตัดสินของเจ้าของโปรเจกต์
...
| `data-series-1..4` | Blue-500 `#026BB5`, Marigold-500 `#F2A81E`, Green-500 `#009148`, Blue-900 `#4B4E5F` | กราฟหลายชุด (เช่น เทียบแผนก) — ดูกฎท้ายข้อนี้ |
...
**กฎสีของกราฟ** *(ตัดสิน 2026-09-11)*

- สีที่ใช้**แยกชุดข้อมูล** มีได้สูงสุด **4 สี** ตามลำดับตายตัว: Blue-500 `#026BB5` → Marigold-500 `#F2A81E` → Green-500 `#009148` → Blue-900 `#4B4E5F` (ห้ามวนซ้ำ ห้ามสลับลำดับ)
- **dark mode** ใช้ขั้นที่สว่างกว่าในบันไดเดียวกัน — ค่าที่เสนออยู่ใน `docs/DESIGN_PLAN.md`
- **ห้ามใช้แดงและเทาเป็นสีชุดข้อมูล** (แดงสงวนไว้สำหรับวิกฤต เทาสำหรับ offline/ชุดที่ถูกลดความสำคัญ)
- กราฟที่มี **มากกว่า 4 ชุด** ต้องเลือกอย่างใดอย่างหนึ่ง: (ก) ใช้สีเดียวทั้งกราฟแล้วใส่ป้ายชื่อกำกับ หรือ (ข) เน้น 1 ชุดด้วยสีลำดับที่ 1 ส่วนที่เหลือใช้ Argent-100 `#CBC7C8`
...
- [ ] สีชุดข้อมูลในกราฟไม่เกิน 4 สีตามลำดับในข้อ 3.3, ไม่มีแดงหรือเทาเป็นสีชุดข้อมูล, กราฟที่เกิน 4 ชุดใช้ทางออกที่ข้อ 3.3 กำหนด
