...
| `/login` | split layout, ฝั่งซ้ายใช้**ภาพจากเอกสาร CI ขององค์กร** (ดูย่อหน้าใต้ตาราง) **ห้ามใช้ภาพจาก template ของ KLC**, การ์ดบัญชีทดลองคงไว้ | 01 |

**ภาพประกอบฝั่งซ้ายของ `/login`** *(ตัดสิน 2026-09-11)*

- แหล่งเดียวคือ **เอกสาร CI ขององค์กร** — ปัจจุบันใช้พาเนล "Sustainability" จากหน้า PDF ที่ 7
  (ห้ามใช้ภาพจาก `01_Login_.png` หรือ template อื่นของ KLC ตามข้อ 2)
- ต้อง extract แล้ววางไว้ที่ `public/brand/` เท่านั้น **ห้าม import ไฟล์ใน `docs/design-refs/` เข้า bundle**
- ต้องทำเป็น **duotone บันไดสี Blue** (เงา = Argent-1000, ไฮไลต์ = Blue-800) ตั้งแต่ตอนสร้างไฟล์
  เพื่อให้**ทุกพิกเซล**มีความสว่างสัมพัทธ์ ≤ 0.1678 → ตัวอักษร Lynx White ได้ ≥ 4.5 : 1 ทั่วทั้งภาพ
  โดยไม่ต้องพึ่ง overlay
  ค่าที่วัดได้จริง: ตัวไฟล์สว่างสุด **0.1304** (→ 5.43 : 1) · เมื่อเรนเดอร์จริงบนพาเนลสว่างสุด **0.1242** (→ **5.63 : 1**)
- **ครึ่งบนของพาเนลต้องไล่เป็นสีทึบ** เพื่อให้โลโก้อยู่บนพื้นเรียบ ไม่ใช่พื้นลาย (ข้อ 5.2)
- ต้องมีสีพื้นสำรอง (`bg-info-strong`) เผื่อภาพโหลดไม่ขึ้น
