...
| 5.1 | ภาพประกอบหน้า `/login` | **ค้าง** — ครอปจาก PDF ได้กว้างแค่ 806 px ซึ่งเป็นความละเอียดสูงสุดที่ไฟล์มี จอ 2x จะเห็นภาพนุ่ม ควรขอภาพต้นฉบับพร้อมกับโลโก้ |
...
## 15. สิ่งที่เปลี่ยนจริงใน Phase 7.2

| เรื่อง | ผล |
|---|---|
| `StatusBadge` | เขียนใหม่ตามข้อ 3.4 — ปกติ/offline ไม่มีพื้น + จุด · เตือน พื้น Marigold-100 + ไอคอน · วิกฤต พื้น Cinnabar-700 + ไอคอนขาว · อัปเกรดให้ 12 จุดเรียกใช้พร้อมกัน |
| `StatusDot` | เปลี่ยนจากวงกลมสีล้วนเป็น **ไอคอนคนละรูปทรงต่อสถานะ** (ติ๊ก / สามเหลี่ยม / แปดเหลี่ยม / วงกลมขีดทับ) |
| ปุ่ม (ข้อ 3.5) | ปุ่มขนาดปกติเป็น 14px จึงใช้พื้น `brand-strong` (Cinnabar-700) · ขนาด `lg` ตั้งเป็น 16px/600 แล้วใช้ Cinnabar-500 ผ่าน `compoundVariants` |
| ผิวและ radius (ข้อ 6.2) | เพิ่ม `--radius-section/card/overlay/control` (24/16/20/12px) · การ์ดเปลี่ยนเป็น 16px และ **ตัดเงาออก** |
| `brand-logo.tsx` | สร้างแล้ว ใช้สัดส่วนจริงของไฟล์ (991 : 707) + clear space = h/10 + ความสูงขั้นต่ำ 32px — ยังไม่เสียบเข้า shell (7.3) |
| opacity หมวดห้าม | แก้ 9 จุดในกลุ่มนี้ — `anomaly-type-badge` ใช้ `border-border` แทนขอบจาง · `diagram-primitives` ใช้พื้นจริงตามน้ำหนักข้อ 3.4 · `pump-card` ใช้ pill วิกฤตพื้นเต็ม |
| `tank-gauge` | ตัวน้ำเปลี่ยนเป็นบันได Blue ตามข้อ 6.3 เลิกผูกกับสถานะ และ **ตัด prop `status` ทิ้ง** (ไม่มีใครใช้แล้ว) |
| สวิตช์ | `settings/field.tsx` และ `control/schedule-panel.tsx` ย้ายจาก `bg-status-ok` มาเป็น `control-checked` |
| `UNKNOWN_ANOMALY_TYPE` | `tone` เปลี่ยนจาก `'warning'` เป็น `'neutral'` — ประกาศเพิ่มใน `anomaly-types.ts` เอง ไม่แตะ `lib/types.ts` |

### บั๊กจาก 7.1 ที่เจอและแก้แล้ว

`components/ai/anomaly-card.tsx:118` — การแทนที่ด้วย `sed` ตอน 7.1 ทิ้งตัว `y` ไว้ท้ายคลาส
กลายเป็น `bg-status-oky` ซึ่ง Tailwind ไม่รู้จัก แท่งคะแนนช่วงต่ำจึงไม่มีพื้นเลย
แก้แล้วและไล่ตรวจทั้งโปรเจกต์ว่าไม่มีคลาสพิมพ์ผิดแบบเดียวกันอีก

### ตัวตรวจอัตโนมัติที่เพิ่มเข้าไป
...
## 17. ภาพประกอบฝั่งซ้ายของหน้า `/login` — *เลิกใช้แล้ว*

> **หมายเหตุ 2026-09-11:** หน้า `/login` ถูกจัดใหม่เป็นการ์ดกลางจอบนภาพฉากที่เจ้าของโปรเจกต์สร้างเอง
> (`public/brand/login-scene.jpg`) ไฟล์ `login-cover.jpg` ที่อธิบายในข้อนี้ถูกลบออกแล้ว
> เก็บวิธีการไว้เพราะยังใช้ได้ ถ้าวันไหนต้องวางตัวอักษรลงบนภาพตรง ๆ อีก


เจ้าของโปรเจกต์ขอให้ใส่ภาพจากเอกสาร CI ขององค์กรเป็นพื้นหลังของข้อความฝั่งซ้าย

| ขั้นตอน | รายละเอียด |
|---|---|
| แหล่ง | `pdfimages -f 7 -l 7` → ภาพหน้าเต็ม 2458 × 1383 แล้วครอปพาเนล "Sustainability" |
| ครอป | x 1652–2458, y 120–1130 → **806 × 1010** (เลี่ยงเลขหน้า "06" และคำบรรยายใต้ภาพ) |
| ปรับสี | แปลงเป็นเทาแล้วทำ duotone ไล่จาก Argent-1000 `#202123` (เงา) ไป Blue-800 `#536281` (ไฮไลต์) |
| ผลลัพธ์ | `public/brand/login-cover.jpg` — 57 KB |

**ทำไมต้องอบสีลงในไฟล์ ไม่ใช้ overlay อย่างเดียว:** เกณฑ์ 4.5 : 1 สำหรับ Lynx White ต้องการให้พื้นหลัง
มีความสว่างสัมพัทธ์ ≤ 0.1678 **ทุกพิกเซล** ถ้าพึ่ง overlay อย่างเดียว จุดที่ภาพสว่างจัด (ท้องฟ้า ใบพัดกังหัน)
จะทะลุขึ้นมาแล้วตัวอักษรอ่านไม่ออกเป็นหย่อม ๆ

**ค่าที่วัดได้จริง** (วัดทุกพิกเซล ไม่ได้สุ่ม):

| วัดที่ไหน | ความสว่างสูงสุด | contrast ของ Lynx White |
|---|---|---|
| ตัวไฟล์ `login-cover.jpg` (อ่านผ่าน canvas) | 0.1304 | 5.43 : 1 |
| พาเนลที่เรนเดอร์จริง (อ่านจากภาพหน้าจอ ตัดตัวอักษรและโลโก้ออก) | **0.1242** | **5.63 : 1** |

*หมายเหตุ:* ตัวเลข 0.1216 / 5.71 : 1 ที่เคยบันทึกไว้รอบแรกมาจากการสุ่มพิกเซลทุก 3 จุดตอนสร้างไฟล์
ซึ่งพลาดจุดที่สว่างที่สุดไป ค่าที่ถูกต้องคือตารางข้างบน (ยังผ่านเกณฑ์ 4.5 : 1 ทั้งคู่)

**การจัดวาง:** ไล่ระดับสีทึบจากบนลงล่าง (100% → 88% → 12% → 42%) ทำให้
โลโก้ที่มุมซ้ายบน**อยู่บนพื้นเรียบ** ตามข้อ 5.2 ส่วนหัวข้อกลางพาเนลอยู่บนภาพที่เห็นชัด
และมี `bg-info-strong` เป็นสีสำรองถ้าไฟล์ภาพโหลดไม่ขึ้น

**ตรวจแล้วว่าโลโก้อยู่บนพื้นเรียบจริง** — วัดค่าเบี่ยงเบนมาตรฐานของความสว่างในแถบรอบโลโก้
จากภาพหน้าจอของ production build ที่ความสูงจอ 700 / 900 / 1200 px ได้ sd = 0.0013 ทั้งสามค่า
(ถ้ามีลายภาพทะลุขึ้นมา sd จะสูงกว่านี้หลายเท่า)

**ข้อจำกัด:** 806 px คือความละเอียดสูงสุดที่มีในไฟล์ PDF จอความละเอียดสูงจะเห็นภาพนุ่ม
บันทึกเป็น open question ข้อ 5.1 ให้ขอภาพต้นฉบับพร้อมกับไฟล์โลโก้

---

## 18. หน้าต่าง "ดูข้อมูลละเอียด" ของกราฟ + ผลตรวจหน่วยความจำ

### 18.1 กราฟทุกตัวกดดูรายละเอียดได้แล้ว

`components/charts/chart-detail.tsx` เป็นตัวกลางตัวเดียวที่ครอบกราฟทุกอัน

| กราฟ | โหมด | ความละเอียดที่เลือกได้ |
|---|---|---|
| `sparkline.tsx` (ใช้ในการ์ดปั๊ม/อุปกรณ์/สภาพแวดล้อม/AI) | เวลา | เลือกอัตโนมัติจากช่วงข้อมูล |
| `daily-usage-chart.tsx` | เวลา | วัน / เดือน / ปี (โหลดย้อนหลัง 400 วันตอนกด) |
| `monthly-chart.tsx` | เวลา | เดือน / ปี (บังคับ เพราะข้อมูลดิบเป็นรายเดือน) |
| `temp-vs-usage-chart.tsx` | เวลา | เลือกอัตโนมัติ |
| `forecast-section.tsx` | เวลา | เลือกอัตโนมัติ |
| `anomaly-timeline.tsx` | เวลา | เลือกอัตโนมัติ |
| `anomaly-evidence-chart.tsx` | เวลา | เลือกอัตโนมัติ |
| `zone-usage-chart.tsx` | หมวดหมู่ | — (แกนนอนเป็นโซน ไม่มีมิติเวลา) |
| `zone-cost-chart.tsx` | หมวดหมู่ | — |

**เลือกความละเอียดอัตโนมัติ** (`autoGrains`) จากช่วงเวลาของข้อมูลจริง
— ไม่เกิน 3 วัน → รายชั่วโมง · ไม่เกิน 14 วัน → ชั่วโมง/วัน · ไม่เกิน 120 วัน → วัน/เดือน · เกินนั้น → วัน/เดือน/ปี
เพราะกราฟจิ๋วในการ์ดมีข้อมูลแค่ 24 ชม. ถ้าให้เลือก "รายปี" จะได้แท่งเดียว ไม่มีประโยชน์

### 18.2 ผลตรวจหน่วยความจำ

CLAUDE.md ระบุว่าห้องคอนโทรลเปิดจอทิ้งไว้ตลอด หน่วยความจำรั่วจึงเป็นบั๊กจริง ไม่ใช่เรื่องรอง
วัดด้วย Chrome headless + `--enable-precise-memory-info --js-flags=--expose-gc`
**บังคับเก็บกวาดก่อนวัดทุกครั้ง** ตัวเลขจึงเป็นหน่วยความจำที่ถูกยึดไว้จริง ไม่ใช่ขยะที่ยังไม่ถูกเก็บ

| การทดสอบ | ผล |
|---|---|
| อยู่หน้าภาพรวมเฉย ๆ 60 วินาที (polling ทุก 2 วิ) | 10.8 → 12.7 MB แล้ว**นิ่ง** |
| สลับ 8 หน้า ครบ 3 รอบ (27 ครั้ง) | 12.8 → 12.8 → 12.8 MB **ไม่ขยับเลย** |
| ปิดหน้าแล้วเก็บกวาด | เหลือ 6.2 MB — คืนหน่วยความจำหมด |

**สรุป: ไม่มีการรั่ว** ถ้า component ที่ถูกถอดออกไม่ปล่อย interval หรือ listener
ตัวเลขรอบที่ 2 และ 3 จะต้องสูงขึ้นเรื่อย ๆ แต่มันนิ่งที่ 12.8 MB

ไล่ดูในโค้ดยืนยันตรงกัน — `setInterval` มีแค่ 3 จุดและมี `clearInterval` ครบทุกจุด
`lib/mock/simulator.ts` นับจำนวนผู้ใช้งาน (`subscriberCount`) แล้วปิด timer เมื่อคนสุดท้ายออก
`use-live-data.ts` คืนฟังก์ชันยกเลิกการติดตามใน cleanup ของ `useEffect`

### 18.3 ขนาด bundle

| | |
|---|---|
| JS ที่ทุกหน้าใช้ร่วมกัน | 87.5 kB |
| หน้าที่หนักสุด (`/ai`) | 276 kB เมื่อโหลดครั้งแรก |
| หน้าที่เบาสุด (`/overview`) | 145 kB |

หน้าที่มีกราฟหนักกว่าหน้าอื่นราว 100 kB เพราะ Recharts — เป็นราคาที่ต้องจ่ายของ stack ที่เลือกไว้ใน CLAUDE.md
